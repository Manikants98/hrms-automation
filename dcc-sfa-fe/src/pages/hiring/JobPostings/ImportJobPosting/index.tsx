...
interface ImportJobPostingProps {
  drawerOpen: boolean;
...
const ImportJobPosting: React.FC<ImportJobPostingProps> = ({
  drawerOpen,
  setDrawerOpen,
}) => {
  ...
  const handleDownloadSample = async () => {
    try {
      await downloadTemplateMutation.mutateAsync('job_postings');
    } catch (error) {
        ...
        <form onSubmit={formik.handleSubmit} className="!space-y-6">
          <Alert severity="info" className="!mb-4">
            <Typography variant="body2">
              Upload an Excel file to import multiple job postings. Download the
...
export default ImportJobPosting;
