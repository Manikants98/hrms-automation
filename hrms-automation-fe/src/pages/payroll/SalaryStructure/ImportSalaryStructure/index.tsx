...
interface ImportSalaryStructureProps {
  drawerOpen: boolean;
...
const ImportSalaryStructure: React.FC<ImportSalaryStructureProps> = ({
  drawerOpen,
  setDrawerOpen,
}) => {
  ...
  const handleDownloadSample = async () => {
    try {
      await downloadTemplateMutation.mutateAsync('salary_structures');
    } catch (error) {
  ...
  return (
    <CustomDrawer
      ...
      title="Import Salary Structures"
      size="medium"
    >
      <Box className="!p-5">
        <form onSubmit={formik.handleSubmit} className="!space-y-6">
          <Alert severity="info" className="!mb-4">
            <Typography variant="body2">
              Upload an Excel file to import multiple salary structures.
...
export default ImportSalaryStructure;
